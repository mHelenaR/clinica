...
class MedicoDAO
{
    ...
    public function insert(MedicoModel $model)
    {
        ...
        $stmt->bindValue(1,(int)$model->medi_crm);
    ...
    public function excluir(MedicoModel $model)
    {
        $sql = "DELETE FROM medico WHERE medi_id_usuario = ?";
        $stmt = $this->conexao->prepare($sql);
        $stmt->bindValue(1, $model->medi_id_usuario);
        $stmt->execute();
    }
}
